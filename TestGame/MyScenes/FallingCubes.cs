...
namespace TestGame.MyScenes
{
    internal class FallingCubes : Scene
    {
        ...
        public FallingCubes(FlyeEngine.FlyeEngine engine) : base(engine)
        {
            ...
            var HP = Engine.AddGameObjectFromWavefront(new Transform() { Rotation = new(0f, -float.Pi / 2f, 0f) }, "MyObjects\\harrypotter", ShaderTypeEnum.SingleColorWithLight);
            ...
            for (int i = 0; i < 6; i++)
            {
                ...
                var cube = Engine.AddGameObjectFromWavefront(transform, "MyObjects\\cube", ShaderTypeEnum.SingleColorWithLight);
                cube.AddRigidBody(2f);
                cube.AddBoxCollider(new Vector3(2.25f), Vector3.Zero);
            }

            _plane =
                Engine.AddGameObjectFromWavefront(new Transform() { Position = new Vector3(-5f, 2f, 0f) }, "MyObjects\\cube", ShaderTypeEnum.SingleColorWithLight);
            ...
            Engine.AddGameObjectFromWavefront(sunTransform, "MyObjects\\sphere", ShaderTypeEnum.SingleColor);
        }
